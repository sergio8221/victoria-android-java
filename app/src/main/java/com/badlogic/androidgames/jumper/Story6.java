...
public class Story6 extends GLScreen{
	Camera2D guiCam;
	...
	public Story6(Game game){
		...
		batcher = new SpriteBatcher(glGraphics,3);
	}
	...
	public void resume(){
		...
		text = new Texture(glGame,"text6.png");
		...
		for(int i=0;i<len;i++){
			...
			if(event.type == TouchEvent.TOUCH_UP){
				if(OverlapTester.pointInRectangle(nextBounds, touchPoint)){
					...
					game.setScreen(new Story7(game));
					return;
				}
	...
	public void present(float deltaTime){
		...
		batcher.drawSprite(185, 240, 280, 280, textRegion);
		batcher.endBatch();
