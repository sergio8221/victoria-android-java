...
public class StoryEnd1 extends GLScreen{
	Camera2D guiCam;
	...
	public StoryEnd1(Game game,int score,int stage){
		super(game);
		puntos=score;
		pantalla=stage;
	...
	public void resume(){
		...
		text = new Texture(glGame,"textend1.png");
		...
		for(int i=0;i<len;i++){
			...
			if(event.type == TouchEvent.TOUCH_UP){
				if(OverlapTester.pointInRectangle(nextBounds, touchPoint)){
					...
					game.setScreen(new VaqueroScreen(game,puntos,pantalla));
					return;
				}
	...
	public void present(float deltaTime){
		...
		for(int i=0;i<2;i++){
			...
			case 0:
				TextureRegion keyFrame = Assets.vaquero2.getKeyFrame(squirrel.stateTime,Animation.ANIMATION_LOOPING);
				float side = squirrel.velocity.x < 0 ? -1:1;
				batcher.drawSprite(squirrel.position.x,squirrel.position.y,side*64,64,keyFrame);
				break;
			case 1:
				TextureRegion keyFrame2 = Assets.victor.getKeyFrame(squirrel.stateTime,Animation.ANIMATION_LOOPING);
