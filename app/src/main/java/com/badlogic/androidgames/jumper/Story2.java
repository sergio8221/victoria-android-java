...
public class Story2 extends GLScreen{
	Camera2D guiCam;
	...
	public void resume(){
		...
		text = new Texture(glGame,"text2.png");
		...
		for(int i=0;i<len;i++){
			...
			if(event.type == TouchEvent.TOUCH_UP){
				...
				if(OverlapTester.pointInRectangle(backBounds, touchPoint)){
					Assets.playSound(Assets.clickSound);
					game.setScreen(new Story1(game));
					return;
				}
	...
	public void present(float deltaTime){
		...
		batcher.drawSprite(170, 180, 240, 240, textRegion);
		batcher.endBatch();
		
		batcher.beginBatch(Assets.items);
		batcher.drawSprite(140, 360, 110, 110, Assets.castle);
		batcher.drawSprite(190, 377, 70, 70, victor);
