...
public class StoryPortero extends GLScreen{
	Camera2D guiCam;
	...
	public StoryPortero(Game game,int score,int stage){
		super(game);
		puntos=score;
		pantalla=stage;
	...
	public void resume(){
		...
		text = new Texture(glGame,"textportero.png");
		...
		for(int i=0;i<len;i++){
			...
			if(event.type == TouchEvent.TOUCH_UP){
				if(OverlapTester.pointInRectangle(nextBounds, touchPoint)){
					...
					game.setScreen(new DiscoScreen(game,puntos,pantalla));
					return;
				}
	...
	public void present(float deltaTime){
		...
		switch(pantalla){
		case 2:
			keyFrame = Assets.rober.getKeyFrame(squirrel.stateTime,Animation.ANIMATION_LOOPING);
			break;
		case 4:
			keyFrame = Assets.alberto.getKeyFrame(squirrel.stateTime,Animation.ANIMATION_LOOPING);
			break;
		case 6:
		default:
			keyFrame = Assets.patri.getKeyFrame(squirrel.stateTime,Animation.ANIMATION_LOOPING);
			break;
		}
		//TextureRegion keyFrame = Assets.rober.getKeyFrame(squirrel.stateTime,Animation.ANIMATION_LOOPING);
