...
public class StoryCastle extends GLScreen{
	Camera2D guiCam;
	...
	public StoryCastle(Game game,int score,int stage){
		super(game);
		puntos=score;
		pantalla=stage;
	...
	public void resume(){
		...
		switch(pantalla){
		case 3:
			text = new Texture(glGame,"textcastle1.png");
			break;
		case 5:
		default:
			text = new Texture(glGame,"textcastle2.png");
			break;
		}
		...
		for(int i=0;i<len;i++){
			...
			if(event.type == TouchEvent.TOUCH_UP){
				if(OverlapTester.pointInRectangle(nextBounds, touchPoint)){
					...
					game.setScreen(new GameScreen(game,puntos,pantalla));
					return;
				}
	...
	public void present(float deltaTime){
		...
		for(int i=0;i<2;i++){
			...
			case 0:
				TextureRegion keyFrame = Assets.mario.getKeyFrame(squirrel.stateTime,Animation.ANIMATION_LOOPING);
				float side = squirrel.velocity.x < 0 ? -1:1;
				batcher.drawSprite(squirrel.position.x,squirrel.position.y,side*64,64,keyFrame);
				break;
			case 1:
				TextureRegion keyFrame2 = Assets.jeni.getKeyFrame(squirrel.stateTime,Animation.ANIMATION_LOOPING);
		...
		default:
			...
			TextureRegion keyFrame = Assets.lucia.getKeyFrame(squirrel.stateTime,Animation.ANIMATION_LOOPING);
			float side = squirrel.velocity.x < 0 ? -1:1;
			batcher.drawSprite(squirrel.position.x,squirrel.position.y,side*64,64,keyFrame);
		}
