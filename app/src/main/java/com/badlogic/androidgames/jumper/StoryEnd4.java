...
public class StoryEnd4 extends GLScreen{
	Camera2D guiCam;
	...
	public void resume(){
		...
		text = new Texture(glGame,"textend4.png");
		...
		for(int i=0;i<len;i++){
			...
			if(event.type == TouchEvent.TOUCH_UP){
				if(OverlapTester.pointInRectangle(nextBounds, touchPoint)){
					...
					game.setScreen(new StoryEnd5(game));
					return;
				}
				if(OverlapTester.pointInRectangle(backBounds, touchPoint)){
					Assets.playSound(Assets.clickSound);
					game.setScreen(new StoryEnd3(game));
					return;
				}
	...
	public void present(float deltaTime){
		...
		batcher.drawSprite(170, 195, 240, 240, textRegion);
		batcher.endBatch();
		
		batcher.beginBatch(Assets.items);
		batcher.drawSprite(155, 365, 70, 70, Assets.blood);
